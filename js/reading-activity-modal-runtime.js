(() => {
  ...
  overlay.hidden = true;
  const introCopy = config.intro || 'Look, listen, and choose the right answer!';
  overlay.innerHTML = `<section class="w1-reading-dialog" role="dialog" aria-modal="true" aria-labelledby="reading-dialog-title"><header class="w1-reading-dialog-header"><div><h2 id="reading-dialog-title">⭐ ${config.title}</h2><div class="w1-reading-intro"><p>${introCopy}</p><button class="w1-reading-speaker" data-intro type="button" aria-label="Listen to the activity instructions">🔊</button></div></div><button class="w1-reading-close" data-close type="button" aria-label="Close activity">✕</button></header><div class="w1-reading-dialog-body"></div></section>`;
  ...
  function makeAnswer(answer) {
    ...
    pick.innerHTML = `<img src="${config.assetBase}${answer.image}" alt="${answer.alt || answer.label}"><span class="w1-reading-answer-label">${answer.label}</span>`;
  ...
  overlay.querySelector('[data-intro]').addEventListener('click', () => speak(introCopy));
