/**
 * Week 1 Golf dress-up
 * One item at a time: Golf uniform → Golf shoes → Golf hat → Golf club → Golf ball
 * Progressive character states on a colorful stage; spoken labels via speechSynthesis.
 */
(function () {
  const VIDEO_SRC = '../assets/video/week-1/golf.mp4';
  const CLIP_SRC = '../assets/video/week-1/Dress%20up/golf-dressup.mp4';
  const ASSET = '../assets/images/week-1/dress-up/golf/';
  ...
  const SUCCESS_LINE = "I'm a golf player!";
  ...
  function buildOverlay() {
    ...
    overlay.id = 'golf-dressup';
    overlay.hidden = true;
    overlay.setAttribute('role', 'dialog');
    overlay.setAttribute('aria-modal', 'true');
    overlay.setAttribute('aria-label', 'Golf dress-up activity');

    overlay.innerHTML = `
      <div class="dressup-modal">
        <div class="dressup-header">
          <h2>⛳ Dress the golf player!</h2>
        ...
        <div class="dressup-stage-column">
          ...
          <aside class="dressup-tray">
            <p class="dressup-word" aria-live="polite">Golf uniform</p>
            <p class="dressup-step">Step 1 of ${STEPS.length} — drag it onto her!</p>
            <div class="dressup-items">
              <button type="button" class="dressup-item" data-piece="uniform" aria-label="Golf uniform">
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const golfBtn = document.querySelector('[data-dress-up="golf"]');
    if (!golfBtn) return;

    golfBtn.addEventListener('click', (event) => {
