/**
 * Week 1 Tennis dress-up
 * One item at a time: Tennis uniform → Tennis Shoes → Tennis Racket → Tennis ball
 * Progressive character states on a colorful stage; spoken labels via speechSynthesis.
 */
(function () {
  const VIDEO_SRC = '../assets/video/week-1/tennis.mp4';
  const CLIP_SRC = '../assets/video/week-1/Dress%20up/tennis-dressup.mp4';
  const ASSET = '../assets/images/week-1/dress-up/tennis/';
  ...
  const SUCCESS_LINE = "I'm a tennis player!";
  ...
  function showComplete() {
    ...
    // Remove the tennis-ball (and any) drag element from the completed screen
    ...
    if (line) line.textContent = "I'm a tennis player!";
  ...
  function advanceAfterPlace() {
    ...
    // Hide the current drag element immediately (especially the tennis ball)
    if (itemBtn) itemBtn.hidden = true;
    if (tray) tray.hidden = true;
    ...
    if (stepIndex >= STEPS.length) {
      // Last piece (tennis ball) done → full fit + ending text
      showComplete();
      return;
    }
  ...
  function buildOverlay() {
    ...
    overlay.id = 'tennis-dressup';
    overlay.hidden = true;
    overlay.setAttribute('role', 'dialog');
    overlay.setAttribute('aria-modal', 'true');
    overlay.setAttribute('aria-label', 'Tennis dress-up activity');

    overlay.innerHTML = `
      <div class="dressup-modal">
        <div class="dressup-header">
          <h2>🎾 Dress the tennis player!</h2>
        ...
        <div class="dressup-stage-column">
          ...
          <aside class="dressup-tray">
            <p class="dressup-word" aria-live="polite">Tennis uniform</p>
            <p class="dressup-step">Step 1 of 4 — drag it onto her!</p>
            <div class="dressup-items">
              <button type="button" class="dressup-item" data-piece="attire" aria-label="Tennis uniform">
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const tennisBtn = document.querySelector('[data-dress-up="tennis"]');
    if (!tennisBtn) return;

    tennisBtn.addEventListener('click', (event) => {
