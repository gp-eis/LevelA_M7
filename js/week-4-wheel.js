(() => {
  ...
  function showSelectedCard(segment){
    ...
    result.textContent=`🎉 You landed on ${segment.label}!`;
    frontImage.src=segment.image; frontImage.alt=segment.label;
    backImage.src=segment.image; backImage.alt=segment.label;
    cardWord.textContent=segment.label;
    cardSentence.textContent=segment.sentence;
    wordCard.style.setProperty('--selected-color',segment.color);
    wordCard.classList.remove('flipped');
    flipHint.textContent='👆 Click the card to flip it!';
    wordCard.setAttribute('aria-pressed','false');
    wordCard.setAttribute('aria-label',`${segment.label} picture card. Click to reveal the sentence.`);
    selectedArea.hidden=false;
    wordCard.focus();
    playContinentWord(segment,false);
  ...
  spinButton.addEventListener('click',() => {
    ...
    spinning=true; stopping=false; lastFrameTime=0;
    result.textContent=''; selectedArea.hidden=true;
    spinButton.disabled=true; spinButton.hidden=true;
  ...
  stopButton.addEventListener('click',() => {
    if(!spinning||stopping)return;
    stopping=true; stopButton.disabled=true; cancelAnimationFrame(animationFrame);
    ...
    function slowDown(time){
      ...
      renderWheel();
      if(progress<1)animationFrame=requestAnimationFrame(slowDown); else finishSpin();
    }
    animationFrame=requestAnimationFrame(slowDown);
  });
  wordCard.addEventListener('click',() => {
    const flipped=wordCard.classList.toggle('flipped');
    flipHint.textContent=flipped?'👆 Click again to see the picture!':'👆 Click the card to flip it!';
    wordCard.setAttribute('aria-pressed',String(flipped));
    wordCard.setAttribute('aria-label',flipped?`${cardSentence.textContent} Click to see the picture again.`:`${cardWord.textContent} picture card. Click to reveal the sentence.`);
    if(currentSegment)playContinentWord(currentSegment,flipped);
  });
